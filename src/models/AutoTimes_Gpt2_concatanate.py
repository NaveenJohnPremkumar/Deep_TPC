...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.mix:
            ...
            # Learnable combination weights for concatenation approach
            #self.combination_weight = nn.Parameter(torch.tensor(0.5))  # Start with equal weights
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, prompt_text=None):
        ...
        if self.mix:
            ...
            # Original approach: addition
            # times_embeds = times_embeds + self.add_scale * x_mark_enc
            # New approach: concatenate along batch dimension (ablation study)
            times_embeds = torch.cat([times_embeds, x_mark_enc], dim=0)
        # outputs: [2 * bs * n_vars x token_num x hidden_dim_of_gpt2] (if mix=True)
        outputs = self.gpt2(
            inputs_embeds=times_embeds).last_hidden_state
        # dec_out: [2 * bs * n_vars x token_num x token_len] (if mix=True)
        dec_out = self.decoder(outputs)
        
        if self.mix:
            # Split back into patch and positional parts
            patch_outputs = dec_out[:bs * n_vars]  # First half
            pos_outputs = dec_out[bs * n_vars:]    # Second half
            # Try different combination strategies:
            # Option 1: Simple addition (current)
            # dec_out = patch_outputs + pos_outputs
            
            # Option 2: Weighted combination (uncomment to try)
            # dec_out = 0.7 * patch_outputs + 0.3 * pos_outputs
            
            # Option 3: Learnable combination (uncomment to try)
            #dec_out = self.combination_weight * patch_outputs + (1 - self.combination_weight) * pos_outputs
            
            # Option 4: Just use patch outputs (uncomment to try)
            dec_out = patch_outputs
            
            # Option 5: Just use positional outputs (uncomment to try)
            # dec_out = pos_outputs
