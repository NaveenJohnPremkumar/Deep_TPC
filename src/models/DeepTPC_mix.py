...
class Model(nn.Module):
    def __init__(self, config):
        ...
        ckpt_dir = os.path.abspath(config.llm_ckp_dir)
        ...
        base_cfg = GPT2Config.from_pretrained(config.llm_ckp_dir)
        base_cfg.mm_layers          = config.mm_layers
        base_cfg.num_fusion_tokens  = config.num_fusion_tokens
        base_cfg.add_cross_attention = True
        base_cfg.num_attention_heads = config.num_attention_heads
        base_cfg.hidden_size         = config.hidden_size
        base_cfg.num_hidden_layers   = config.num_hidden_layers
        base_cfg.layer_norm_epsilon  = config.layer_norm_epsilon
        base_cfg.attn_pdrop          = config.attn_pdrop
        base_cfg.resid_pdrop         = config.resid_pdrop
        base_cfg.embd_pdrop          = config.embd_pdrop

        # self.gpt2 = GPT2ModelWithMM(base_cfg)

        self.gpt2 = GPT2ModelWithMM.from_pretrained(config.llm_ckp_dir, config=base_cfg)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, prompt_text):
        ...
        if self.mix:
            ...
            mark_embeds = self.add_scale * x_mark_enc
            # Reshape mark_embeds to match batch size
            mark_embeds = mark_embeds.view(bs, n_vars * token_num, -1)  # [B, n_vars*token_num, D]
        else:
            mark_embeds = None
        ...
        outputs = self.gpt2(
            ...
            patch_embeddings=mark_embeds if self.mix else times_embeds,  # Use mark_embeds for cross attention if mixing
